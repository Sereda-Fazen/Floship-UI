...
import requests
__all__ = ['get_email_address', 'get_mailbox', 'get_mail_by_subject']
...
class TempMail(object):
    ...
    def get_mailbox(self, email=None, message_id=None, email_hash=None ):
        """
        Return list of emails in given email address
        or dict with `error` key if mail box is empty.
        :param email: (optional) email address.
        :param message_id: (optional) id of a message.
        ...
        if message_id is None:
            return req.json()
        return req.json()[message_id]

    def get_mail_by_subject(self, email=None, subject=None ):
        """
        Return email with given subject
        or dict with `error` key if mail box is empty.
        :param email: (optional) email address.
        :param message_id: (optional) id of a message.
        :param email_hash: (optional) md5 hash from email address.
        ...
        url = 'http://{0}/request/mail/id/{1}/format/json/'.format(
            self.api_domain, email_hash)
        req = requests.get(url)
        if req.json() != {'error': 'There are no emails yet'}:
            for item in req.json():
                if item['mail_subject'] == subject:
                    return item

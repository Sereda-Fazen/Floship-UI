...
function find_binary {
    ...
    if [ ${KERNEL_NAME} == 'Linux' ]; then
        ...
        PATHS=(/opt/google/chrome/google-chrome /usr/local/bin /usr/bin/google-chrome)
...
if [ -n "$DEV_TOOLS_PROXY_BINARY" ]; then
    ...
    ${DEV_TOOLS_PROXY_BINARY} --port ${PROXY_DEBUGGING_PORT} --chrome-port ${CHROME_DEBUGGING_PORT} ${DEV_TOOLS_PROXY_ARGS} > /dev/null 2>&1 &
    PROXY_PID=$!
    CHROME_PID=$$
